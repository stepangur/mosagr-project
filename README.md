...
#### 12.2. Настройте DNS-записи у регистратора домена

Войдите в панель управления DNS у вашего регистратора (RU-CENTER, REG.RU, Timeweb, Beget и т.д.) и создайте следующие записи:

| Тип | Имя | Значение | TTL |
|-----|-----|----------|-----|
| `A` | `@` (корень) | `185.123.45.67` | 3600 |
| `A` | `www` | `185.123.45.67` | 3600 |

> **Что такое `@`:** символ `@` означает корневой домен, то есть `razrabotka-agr.ru`.  
> **IP-адрес** замените на реальный IP вашего сервера.

#### 12.3. Подождите распространения DNS

После сохранения записей DNS-изменения распространяются от **5 минут до 24 часов** (обычно 15–60 минут).

Проверьте, что домен уже указывает на ваш сервер:

```bash
# Проверить A-запись
nslookup razrabotka-agr.ru
# или
dig razrabotka-agr.ru +short

# Должен вернуть IP вашего сервера: 185.123.45.67
```

Онлайн-проверка: [dnschecker.org](https://dnschecker.org) — позволяет увидеть распространение DNS по всему миру.

> ⚠️ **Не переходите к следующим шагам, пока DNS не распространился.** Certbot (SSL) проверяет доступность домена — если домен ещё не указывает на сервер, получение сертификата завершится ошибкой.
